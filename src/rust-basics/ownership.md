...
Rust guarantees the absence of memory leaks and does so without a garbage collector. To achieve that the compiler relies on the mechanism called **ownership**.

In Rust any object of a non-primitive type must have exactly one owner. The **owner** is the variable to which the object is assigned. Therefore, when the value of one variable is assigned to another, the ownership is transferred from the first variable to the second, making the first variable invalid.
...
The fact that every object has only one owner allows the compiler to understand where exactly the memory occupied by the object should be freed (usually this happens at the end of the scope which constains the owner variable). This allows the compiler to inserts a code that frees the memory owned by a variable to the place where the variable gets destroyed.

This approach allows to manage memory without a garbage collector and guarantees that there won't be memory leaks.

The mechanism of the Rust compiler that tracks object lifetimes (and ensures memory is cleared correctly) is called **borrow checker**.

## Ownership and Scopes

As we already know, the lifetime of a variable is tied to the scope in which it is declared: when the scope ends, all variables within it disappear, and the memory they owned is freed. However, a variable declared inside a scope can "give" its data to another variable declared outside that scope.
...
Now that we understand that passing a variable as an argument effectively destroys that variable (because ownership is moved), let’s look at an example that seems completely normal in most programming languages but may become a surprise for people who learn Rust.
...
How can we solve this problem? Let's write an absurd but demonstrative solution. As we've already know, with tuples we can return multiple values from a function. That means we can return the object passed as an argument back to the caller.
...
> We wrote the function this way only to demonstrate borrowing.
...
The question is: what does the reference, that pointed to the second element, now point to? Obviously, such a reference would become invalid.

Fortunately, Rust is a safe language, so the compiler will not allow you to write this code. The rule that prevents such invalid references to appear, stands:
...
5 |   let r2 = & s;    // <-- attempting to make an immutable reference
...
All the ownership principles described above do not apply to primitive types.

Primitive types occupy little memory and do not own additional resources. That's why, in situations where ownership would be transferred for composite types, primitive types are simply copied.
...
This problem is solved just as easily as the earlier issue with passing object as a function argument — by taking a reference.
...
Now that we replaced `arr` with `&arr` in the for loop header, each iteration assigns to `n` not the next element itself, but a reference to it. And because of that, the array is not destroyed.

> [!NOTE]
> After this example, it might seem that writing Rust programs turns into a constant battle with the compiler. At first, that may indeed be the case. However, once you get used to the concept of ownership, you will naturally start writing code the correct way.
