...
Traits are used to define a type interface by defining a list of methods the type should have. If a type implements a trait, then this type must have implementations for <ins>all</ins> methods declared in that trait.
...
```rust
// A trait describes a type that can introduce itself
trait CanIntroduce {
...
impl CanIntroduce for Person {
    fn introduce(&self) -> String {
        // A person introduces himeself by stating his name
...
Of course, traits aren't used just for the sake of implementing them. The primary use of traits is writing polymorphic code — the code that interacts with types not directly, but through the traits they implement. For example, you can make a function that accepts as an argument an object of any type that implements the required trait.

In Rust, there are two fundamentally different approaches to passing arguments based on traits:

* Static dispatch — when the argument type is specified as `impl Trait`
* Dynamic dispatch — when the argument type is specified as `dyn Trait`
...
To accept an argument via a trait using static dispatch, you must specify the argument type as `impl Trait`.
...
Since we are accepting the argument by its trait, the only thing we can do with it is what is declared in the trait. But that is exactly what we need.

To demonstrate how this function can accept arguments of different types, let's additionally create a `Dog` and implement the `CanIntroduce` trait for it as well.
...
After finding a call of `print_introduction` function for `Person` and then for `Dog`, the compiler will generate something like the following (the actual internal names will differ, of course):
...
Thus, at the compilation stage, each generated version of the `print_introduction` function already knows exactly which concrete type it is working with. Consequently, each monomorphization version of `print_introduction` knows the <ins>address</ins> of the implementation of `introduce` method it needs.
...
As we've mentioned, the difference is in internals. With static dispatch, the compiler generates a separate version of the function for every distinct type it is called with. In dynamic dispatch, only one version of the function exists.

How does `print_introduction` know which implementation of `CanIntroduce` to use and where to find the correct `introduce` method? The secret lies in the fact that the `&dyn CanIntroduce` argument isn't just a simple reference to the object. It is a pair of pointers: the first points to the object itself, and the second points to a **vtable** (virtual method table) for that specific type. In Rust terminology, this pair is known as a **fat pointer**.

When the compiler encounters code where a concrete type is accessed via `dyn Trait`, it generates (if it isn't already generated) a unique vtable for that type. This table contains a mapping from method names (defined in the trait) to exact method address in the code segment.

At every polimorphic functiona call where an arguments is passed as `dyn Trait`, the compiler generates code that passes a "fat pointer" (the object's address + the vtable address).

Inside the polimorphic function itself, at each point where a method is called on the `dyn Trait` agrument the compiler inserts logic to:
...
> 
> Just as a slice reference `&[T]` is not just a pointer to the start of a sequence but a pointer plus a length, a `&dyn T` reference is not just a pointer to data, but a pointer to data plus a pointer to a vtable.

### `impl` vs `dyn`

Let's summarize. There are two ways to refer to a type via a trait it implements:

* `impl Trait` — is replaced by a concrete type during compilation.
* `dyn Trait` — is replaced by a trait object that proxies method calls to the real type using dynamic dispatch.
...
In practice, this means that if we want to implement trait `A` for our type, we also need to implement trait `B` for it.
...
Let’s look at passing arguments via `impl Trait` from a different angle. When we specify a trait as a function argument, we are effectively imposing a bound (restriction) on the types that can be passed to that function.
...
It is worth mentioning that this specific syntax for specifying multiple trait bounds is rarely used in this form. Instead, developers typically use a different syntax which we will explore in the [Generics](generics.md) chapter.
...
In this implementation of `HasDefaultConstructor` for `Person`, we specified the return type of `make_default` as `Self`. However, we also could used `Person` explicitly: the effect is identical.
