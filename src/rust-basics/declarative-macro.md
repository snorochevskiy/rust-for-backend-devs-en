...
* **Declarative Macros**. These macros are processed after the program's source code has been transformed into an AST (Abstract Syntax Tree). They manipulate AST nodes, which makes them relatively easy to write and quite safe. Essentially, declarative macros are functions that run at compile time and manipulate AST nodes.
...
Declarative macros are defined using the `#[macro_export]` annotation, followed by the macro description that is written using special Scheme-like syntax.
...
Let's look at a simple macro that sums two numbers:
...
`( $x:expr, $y:expr )` means we expect two arguments, each of them must be a valid Rust expression (the `expr` type stands for expression).
...
| `item`    | An item: `struct Point {x: f64, y: f64}`, `mod mymod;`                | anything         |
| `block`   | A code block / scope:: `{ s+= "ok"; true }`                           | anything         |
| `meta`    | An attribute body: `inline`, `derive(Copy,Clone)`, `doc="3d models."` | anything         |
| `ident`   | An identifier: `std`, `Json`, `my_var`                                | anything         |
| `tt`      | A token tree: `;`, `>=`, `{}`, `[0 1 (+ 0 1)]`                        | anything         |
| `literal` | A literal: `5`, `5u32`, `1.0`, `"Hello"`                              |                  |
| `vis`     | Visibility qualifier: `pub`, `pub (crate)`                            |                  |

As you can see, thanks to the type system for arguments, Rust macros are much safer than macros in C.
...
The first pattern `() => { 0 }` is simple: it handles the case where no values are passed to the macro. In this case, we simply return `0`.
...
You can use whichever variant feels more expressive.

The only technical difference is the need for a semicolon `;` after the call. If a macro is used to generate a function or a struct, you don't need a semicolon if you use curly braces `{}`.
