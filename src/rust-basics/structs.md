...
Like variables, structure fields must be named using snake_case. A structure itself should be named using PascalCase (the name starts with a capital letter, and if there are multiple words, each word also starts with a capital letter), e.g. `User`, `MainAddress`, or `DatabaseConnection`.

Example of a structure that is used to store a person's first and last name:
...
Structure fields are accessed using the dot operator: `struct.field`.
...
Structs variables, same as other variables, are immutable by default.
To change struct's field values, the entire struct variable must be declared as mutable using `mut`.
...
With the help of `..` operator, you can create a new object from an existing one by changing only specific fields:
...
Unlike traditional OOP languages where methods are declared within the class body, in Rust methods are declared outside of the structure in a `impl` block.
...
impl StructName {
  ...
  // method, that does NOT chage the caller object
  fn method_1(&self, arg_1: Type1, …, arg_N: ТипN) -> ResultType {
    ...
  }

  // method, that DOES CHANGE the caller object
  fn method_2(&mut self, arg_1: Type1, …, arg_N: TypeN) -> ResultType {
    ...
  }

  // "static" method, that is called on the strcuture, not on the object
  fn method_3(arg_1: Type1, …, arg_N: TypeN) -> ResultType {
    ...
  }
}
```
...
These are useful when you need to implement a trait on a type but don't need to store any data within the type itself. We will learn about traits later in the [Traits](traits.md) chapter.

## Lifetimes in Structures

In the [Lifetime](lifetimes.md) chapter, we learned what lifetimes are and how to define them for functions. Now, let’s explore how lifetimes work with structs.
