...
An **anonymous function** is a function that acts as an object: it can be assigned to a variable or passed as an argument to another function.
...
> [!TIP]
> Anonymous functions are often called "lambda expressions," referring to the branch of mathematics known as "Lambda Calculus," which serves as the foundation for functional programming.
> 
> Anonymous functions are also frequently called "lambda functions."
...
Anonymous functions are compiled in the same way as regular functions and are placed in the code segment. When we assign an anonymous function to a variable, we are simply storing <ins>a pointer to the function</ins> located in the code segment.
...
Unfortunately, this code fails to compile with the error:
...
The issue is that from within the body of our anonymous function, we are accessing data that belongs to a scope outside of the function. Such type of anonymous function that depends on a outer data is called a **closure**.
