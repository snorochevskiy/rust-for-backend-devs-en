...
> [!NOTE]
> If this chapter feels difficult, don't try to understand it completely. Feel free to come back to it after reading the chapters about [Ownership](ownership.md) and [Structs](structs.md).
...
Essentially, [&str](https://doc.rust-lang.org/std/primitive.str.html) is a slice pointing to a buffer that contains a sequence of UTF-8 encoded characters. (In some ways, `&str` is similar to `const char*` strings in the C language)\
And since `&str` is a slice, it is stored in memory as a pair of values: address of the buffer and its length.

When the compiler encounters a string literal in the code, e.g. `let s = "abc"` it typically places that string (sequence of character itself) in the data segment (or code segment, depending on the target platform), where it "lives" from the start of the program until it ends. And the varibale `a` is assigned only with a slice that refers to `"abc"` characters.

The `&str` type does not manage the memory where the string resides, it only references the data. This memory can belong to the data segment, the heap, or even reside on the stack.
...
Technically, [String](https://doc.rust-lang.org/std/string/struct.String.html) is a wrapper around a `Vec<u8>` byte-vector that stores a sequence of UTF-8 encoded characters. Because of this, `String` always has sole ownership of its string buffer, and this buffer is always located on the heap.
...
The most straightforward way to create a String object is the `String::from(&str)` constructor-function (we'll discuss them more in the [Structs](structs.md) chapter). This function:

1. Creates a `String` object, which, as mentioned before, is a wrapper around a `Vec<u8>`.
2. Copies the string characters from the `&str` argument into the newly created vector.
...
> [!NOTE]
> Obviously, it's much shorter and simpler to just write `String::from("text")`. In the example above, we created the `slice` variable separately only for the sake of clarity.
...
The last way to create a `String` is by calling the `.to_string()` method on an `&str` slice. Essentially, this method does exactly the same thing as `String::from(&str)`, just with different syntax.
...
```rust
fn main() {
  // The compiler sees a string literal and places 
  // its data in the static data segment.
...
We are already familiar with the `println!` macro used for console output. As the first argument this macro takes a format string that allows to "inject" values using `{}`.
