...
Consider the following example. We will try to create a struct to store the coordinates of a point in 2D space. Then we attempt to compare two instances of the point for equality:
...
Thus, to make equality check available for `Point2D` struct, we must implement `PartialEq` for it:
...
If we think about the implementation of the `eq` method for our point, which simply consists of comparing all fields, we'll notice that for the vast majority of structs, comparison will also simply involve comparing all corresponding fields.
...
If we add the `#[derive(PartialEq)]` attribute to our struct, the compiler will automatically generate the `PartialEq` implementation for our type. The automatic implementation of the `eq` method simply compares all corresponding fields of the struct objects, which is exactly what we need.
...
We've already discussed the `derive` attribute, but we aren't yet familiar with attributes themselves.
...
When the compiler encounters an attribute, it turns to the corresponding handler, which performs some code generation.
...
In addition to `PartialEq`, there are other standard traits that support implementation generation, e.g.:
...
* [Default](https://doc.rust-lang.org/std/default/trait.Default.html) — allows creating a default value. For example: `0` for numbers, `false` for booleans, an empty string for strings, and so on.
...
However, if we want to make possible to copy the object instead of moving it, we should implement [Clone](https://doc.rust-lang.org/std/clone/trait.Clone.html) trait, that provids the `clone()` method. The trait itself looks like this:
...
However, the beauty is that for `Clone` trait, an implementation can also be generated simply by adding the `derive` attribute.
...
When the compiler processes an assignment operation, it checks the type of the object beein assigned. If it implements `Copy` trait, then instead of moving the object, it calls `clone()` to make a copy.
...
Now, when assigning the variable `p1` to the variable `p2`, ownership transfer does not happen. Instead, a copy of the object from variable `p1` is created and assigned to variable `p2`.
