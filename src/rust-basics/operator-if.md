...
It is important to note here that for the `if` operator to return a value, the last expression in the conditional branch <ins>must not</ins> end with a semicolon (`;`). This is because the body of a conditional branch is always a scope. And as we already know from the [Scopes](scopes.md) chapter, if the last expression in a scope ends with a `;`, the scope returns `()`.
