...
In addition to the `if` statement, Rust has another conditional operator that we haven't covered yet — the `match` operator, often referred to as **pattern matching**.
...
In reality, the `_` pattern is just a regular "discarded" variable, similar to those we saw in destructuring patterns. Instead of a "discarded" variable, you can specify a variable with any valid name, and the value passed into `match` will then be bound to that variable.
...
Keep in mind that when the pattern is represented by just a variable (like this `x`), it matches <ins>any value</ins>, so it must be placed at the very end of the pattern list.
...
fn main() {
    ...
    // Prints: The number is less than 10
}
```

For numberical values, instead of listing all values manually, you can specify a range:
...
* contain multiple conditions combined via `||` and `&&` operators.
...
However, there is an alternative syntax — the `ref` keyword. It allows you to bind an object's field by reference within a destructuring pattern, even if the object was passed to the `match` by value. If all fields are captured via `ref` (or ignored), the original object is not consumed.
